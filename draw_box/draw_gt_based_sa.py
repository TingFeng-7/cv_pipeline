...
if __name__ == '__main__':
    # import cv2
    # imgsdir = r'D:\sa_data\sa_coco_1' #图片根目录 
    # imgsdir = r'D:\sa_data\0010_v1.5_filtered_linebox_yolox'
    imgsdir = r'D:\sa_data\0006_sa_v1.6_cleanto_0010\0001_cyclone_1.5'
    ...
    label_dir = r'D:\20230106_valscaled32_f27_yolox-nano-fpn4\Element_grabbing\grabbing_evaluation' #预测json目录
    imgs_link = r'imgs'
    ...
    # debug_name = 'coco2SAcomps_debug'
    debug_name = '4_debug'
    from loguru import logger
    #--2.以上是类别名的处理
    # app_names = ['labelme']
    for app in app_names:
        # app_folder = os.path.join(imgsdir, app) #app 文件夹
        app_folder = imgsdir 
        if (label_dir == 'gold'): #不设置预测结果的路径的话，默认在一起
            predict_debug_folder = os.path.join(app_folder, debug_name)
            predict_folder = os.path.join(app_folder, 'coco2SAcomps')#json pos

        else:
            label_folder = os.path.join(label_dir, app)#每个预测里有imgs
            predict_debug_folder = os.path.join(label_folder, debug_name)
            predict_folder = os.path.join(label_folder, predict_link)#json pos
        
        # img_folder = os.path.join(app_folder, imgs_link)
        img_folder = r'D:\sa_data\0010_v1.5_filtered_linebox_yolox\labelme\imgs'
