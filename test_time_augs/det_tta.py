...
def mask_flip(masks:Tensor, 
                img_shape: Tuple[int],
                direction='horizontal'):
    
    h, w = img_shape #只能在一个尺度下面
    assert direction in ['horizontal', 'vertical', 'diagonal']
    flipped_masks = masks.clone()
    
    if direction == 'horizontal':
        flipped_masks = flipped_masks[:, :, ::-1] 
    elif direction == 'vertical':
        flipped_masks = flipped_masks[:, ::-1, :]
    else:
        flipped_masks = flipped_masks[::-1, ::-1, :]
...
class DetTTAModel(BaseTTAModel):
    ...
    def merge_aug_bboxes_masks(self, aug_bboxes: List[Tensor],
                         aug_scores: List[Tensor],
    ...
    def _merge_single_sample(
        ...
        for data_sample in data_samples: # 1张图片 不同角度
        ...
        if data_samples[0].pred_instances.get('masks', None):
            merged_bboxes, merged_scores, merged_masks = self.merge_aug_bboxes_masks(
                aug_bboxes, aug_scores, aug_masks, img_metas)
        else:
            # 融合 框 和 得分
